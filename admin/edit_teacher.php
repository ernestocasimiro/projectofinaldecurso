...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmtCurrent = $conn->prepare("SELECT foto_bi1, foto_bi2, fotoperfil FROM professores WHERE id = ?");
    ...
    if (empty($errors)) {
        try {
            ...
            $errors[] = "Erro ao atualizar professor: " . $e->getMessage();
        }
    }
} else {
...
<title>Editar professor - <?= htmlspecialchars($g['fname'] . ' ' . $g['lname']); ?></title>
<style>
    body {
...
<div class="container">
    ...
    <form action="edit_teacher.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
       ...
       <label for="especializacao">Especialização</label>
            <input 
                ...
                id="especializacao" 
                name="especializacao" 
                value="<?= htmlspecialchars($_POST['especializacao'] ?? $g['especializacao'] ?? '') ?>" 
                required
                placeholder="Digite a especialização do professor"
            />
        ...
        <div class="btn-group">
            ...
            <a href="teacher.php" class="btn-cancel">Cancelar</a>
        </div>
    </form>
</div>
</body>
