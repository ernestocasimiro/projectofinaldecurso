...
if (isset($_SESSION['id']) && isset($_SESSION['role'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
        ...
        try {
            $checkTelStmt = $conn->prepare("SELECT id FROM coordenadores WHERE telefone = ?");
            $checkTelStmt->execute([$telefone]);
            if ($checkTelStmt->rowCount() > 0) {
                $_SESSION['error'] = "Este telefone já está em uso por outro coordenador.";
                header("Location: coordinator.php");
                exit;
            }
        ...
        try {
            $checkBISql = $conn->prepare("SELECT id FROM coordenadores WHERE num_bi = ?");
            $checkBISql->execute([$numbi]);
            if ($checkBISql->rowCount() > 0) {
                $_SESSION['error'] = "Este número do BI já está cadastrado para outro coordenador.";
                header("Location: coordinator.php");
                exit;
            }
        } catch (PDOException $e) {
        ...
        function generateUsername($fname, $lname, $conn) {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) FROM coordenadores WHERE username = ?");
        ...
        $sql = "INSERT INTO coordenadores (
            ...
            fotoperfil, telefone, email, area_coordenacao, nivel_academico, anos_experiencia,
            password, username
        ) VALUES (
        ...
        try {
            $stmt->execute();
            $_SESSION['success'] = "Coordenador adicionado com sucesso! Nome de usuário: <strong>$username</strong>";
        } catch (PDOException $e) {
            $_SESSION['error'] = "Erro ao adicionar coordenador: " . $e->getMessage();
        }
    ...
    try {
        $listStmt = $conn->prepare("SELECT id, fname, lname, genero, telefone, email, nivel_academico, anos_experiencia, area_coordenacao FROM coordenadores ORDER BY fname ASC");
        $listStmt->execute();
        $coordinator = $listStmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        $coordinator = [];
        $_SESSION['error'] = "Erro ao buscar coordenadores: " . $e->getMessage();
    }
...
<head>
  ...
  <title>Coordenadores - Sistema Pitruca Camama</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <!-- Manter todos os estilos CSS da tela de coordenadores -->
  <style>
    ...
    <main class="content">
        <header>
            <div class="search-bar">
                ...
                <input type="text" placeholder="Pesquisar coordenadores..." id="search-coordinator">
            </div>
            <div class="user-info">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" id="register-coordinator">
    ...
    <table class="data-table">
        ...
        <tbody id="guardian-table-body">
            <?php if (!empty($coordinator)): ?>
                <?php foreach ($coordinator as $g): ?>
                    <tr>
                        <td><?= htmlspecialchars($g['id']); ?></td>
                        <td><?= htmlspecialchars($g['fname'] . ' ' . $g['lname']); ?></td>
                        <td><?= htmlspecialchars($g['area_coordenacao'] ?? ''); ?></td>
                        <td><?= htmlspecialchars($g['nivel_academico'] ?? ''); ?></td>
                        <td><?= htmlspecialchars($g['anos_experiencia'] ?? ''); ?></td>
                        ...
                        <div class="action-buttons">
                          <button class="view-btn" data-id="<?= $g['id']; ?>" onclick="window.location.href='view_coordinator.php?id=<?= $g['id']; ?>'">
                            <i class="fas fa-eye"></i>
                          </button>

                          <button class="edit-btn" onclick="window.location.href='edit_coordinator.php?id=<?= $g['id']; ?>'">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="7">Nenhum coordenador cadastrado ainda.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
                ...
                <!-- Paginação -->
                <div class="pagination" id="coordinator-paginacao">
...
<!-- Modal para adicionar/editar coordenador -->
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-container">
                <form id="formCoordinator" action="coordinator.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="anos_experiencia" name="anos_experiencia" min="0" max="50" required>
                        </div>
                        
                        <div class="file-input-container">
                            <label for="fotoperfil">Foto do Coordenador*</label>
...
<!-- Modal para visualizar coordenador -->
<div class="modal" id="view-coordinator-modal">
    <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
            <div id="coordinator-details">
                <!-- Detalhes do coordenador serão carregados via JavaScript -->
            </div>
            <div class="form-buttons">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Controle do modal de coordenador
    const coordinatorModal = document.getElementById('coordinator-modal');
    const viewCoordinatorModal = document.getElementById('view-coordinator-modal');
    ...
    // Abrir modal de adicionar coordenador
    if (addCoordinatorBtn) {
        addCoordinatorBtn.addEventListener('click', function() {
            ...
            document.getElementById('formCoordinator').reset();
            clearImagePreviews();
    ...
    closeModalBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            viewCoordinatorModal.style.display = 'none';
        });
    });
    ...
    window.addEventListener('click', function(e) {
        ...
        if (e.target === viewCoordinatorModal) {
            viewCoordinatorModal.style.display = 'none';
        }
    });
    ...
    const coordinatorForm = document.getElementById('formCoordinator');
    ...
    // Função para visualizar coordenador
    function viewCoordinator(id) {
        // Aqui você faria uma requisição AJAX para obter os detalhes do coordenador
            ...
            <div class="coordinator-profile">
                <div class="coordinator-header">
                    <div class="coordinator-avatar">
                        <img src="uploads/coordinator/default-avatar.jpg" alt="Foto do Coordenador">
                    </div>
                    <div class="coordinator-basic-info">
                        <h2>Maria Fernandes</h2>
                        <p><strong>ID:</strong> ${id}</p>
                        <p><strong>Área de Coordenação:</strong> Pedagógica</p>
                        <p><strong>Status:</strong> <span class="status-badge status-active">Ativo</span></p>
                    </div>
                </div>
                
                <div class="coordinator-details-grid">
                    ...
                    <div class="detail-section">
                        ...
                        <p><strong>Email:</strong> maria.fernandes@email.com</p>
                    </div>
                    
                    <div class="detail-section">
                        <h3>Informações Profissionais</h3>
                        <p><strong>Nível Acadêmico:</strong> Mestrado</p>
                        <p><strong>Anos de Experiência:</strong> 8</p>
                        <p><strong>Área de Coordenação:</strong> Pedagógica</p>
                    </div>
                    
                    <div class="detail-section">
                        <h3>Informações Adicionais</h3>
                        <p><strong>Data de Contratação:</strong> 10/01/2010</p>
                        <p><strong>Observações:</strong> Coordenadora dedicada com excelente desempenho.</p>
                    </div>
                </div>
            </div>
        `;
        
        document.getElementById('coordinator-details').innerHTML = coordinatorDetails;
        viewCoordinatorModal.style.display = 'block';
    }
    
    // Função para editar coordenador
    function editCoordinator(id) {
        // Aqui você faria uma requisição AJAX para obter os dados do coordenador
        ...
        document.getElementById('fname').value = 'Maria';
        document.getElementById('lname').value = 'Fernandes';
        ...
        document.getElementById('email').value = 'maria.fernandes@email.com';
        document.getElementById('area_coordenacao').value = 'Pedagógica';
        document.getElementById('nivel_academico').value = 'Mestrado';
        document.getElementById('anos_experiencia').value = '8';
        
        // Exibir o modal
        coordinatorModal.style.display = 'block';
    }
    
    // Função para excluir coordenador
    function deleteCoordinator(id) {
        if (confirm(`Tem certeza que deseja excluir o coordenador com ID ${id}?`)) {
            // Aqui você faria uma requisição AJAX para excluir o coordenador
            alert(`Coordenador com ID ${id} excluído com sucesso!`);
    ...
    // Filtros e pesquisa
    const searchInput = document.getElementById('search-coordinator');
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            filterCoordinatorTable(searchTerm, filterArea.value, filterStatus.value);
        });
    }
