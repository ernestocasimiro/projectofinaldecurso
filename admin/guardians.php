...
if (isset($_SESSION['id']) && isset($_SESSION['role'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
        ...
        $genero      = $_POST['genero'];
        $dataa       = $_POST['dataa'];
        $numbi       = strtoupper(trim($_POST['numbi']));
        ...
        if (!preg_match('/^\d{9}$/', $telefone)) {
            $_SESSION['error'] = "Número de telefone inválido. Deve conter 9 dígitos.";
            header("Location: guardians.php");
            exit;
        }
        ...
        function generateUsername($fname, $lname, $conn) {
            ...
            $stmt = $conn->prepare("SELECT COUNT(*) FROM encarregados WHERE username = ?");
        ...
        $username = generateUsername($fname, $lname, $conn);
        ...
        if (!$foto_bi1 || !$foto_bi2 || !$fotoperfil) {
            $_SESSION['error'] = "Erro no upload dos arquivos (foto do BI ou perfil).";
            header("Location: guardians.php");
            exit;
        }

        // Verifica se já existe o mesmo número de BI
        $checkStmt = $conn->prepare("SELECT id FROM encarregados WHERE num_bi = ?");
        $checkStmt->execute([$numbi]);

        if ($checkStmt->rowCount() > 0) {
            $_SESSION['error'] = "Já existe um encarregado cadastrado com este número de BI.";
            header("Location: guardians.php");
            exit;
        }

        // Inserir no banco
        $sql = "INSERT INTO encarregados (
            fname, lname, genero, data_nascimento, num_bi, foto_bi1, foto_bi2,
            endereco, fotoperfil, telefone, email, profissao, parentesco, username, password
        ) VALUES (
            :fname, :lname, :genero, :dataa, :numbi, :foto_bi1, :foto_bi2,
            :endereco, :fotoperfil, :telefone, :email, :profissao, :parentesco, :username, :password
        )";
        ...
        try {
            $stmt->execute();
            $_SESSION['success'] = "Encarregado adicionado com sucesso! Nome de usuário: <strong>$username</strong>";
        } catch (PDOException $e) {
            $_SESSION['error'] = "Erro ao adicionar encarregado: " . $e->getMessage();
        }
    ...
    // --- BUSCAR LISTAGEM DE ENCARREGADOS ---
    try {
        $listStmt = $conn->prepare("SELECT id, fname, lname, genero, telefone, email, parentesco, profissao, username FROM encarregados ORDER BY fname ASC");
        $listStmt->execute();
        $guardians = $listStmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        $guardians = [];
        $_SESSION['error'] = "Erro ao buscar encarregados: " . $e->getMessage();
    }
?>
...
<head>
  ...
  <title>Encarregados - Sistema Pitruca Camama</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <!-- Manter todos os estilos CSS das telas anteriores -->
  <style>
    ...
    <main class="content">
        <header>
            <div class="search-bar">
                ...
                <input type="text" placeholder="Pesquisar encarregados..." id="search-guardians">
            </div>
            <div class="user-info">
    ...
    <div class="tab-pane active" id="register-guardians">
        ...
        <div class="table-filters">
            <div class="filter-group">
                <label for="filter-parentesco">Filtrar por Parentesco</label>
                <select id="filter-parentesco">
...
<div class="table-container">
    <table class="data-table">
            ...
            <?php if (!empty($guardians)): ?>
                <?php foreach ($guardians as $g): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($g['parentesco'] ?? ''); ?></td>
                        <td><?= htmlspecialchars($g['profissao'] ?? ''); ?></td>
                        ...
                        <div class="action-buttons">
                          <button class="view-btn" data-id="<?= $g['id']; ?>" onclick="window.location.href='view_guardian.php?id=<?= $g['id']; ?>'">
                            <i class="fas fa-eye"></i>
                          </button>

                          <button class="edit-btn" onclick="window.location.href='edit_guardian.php?id=<?= $g['id']; ?>'">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="7">Nenhum encarregado cadastrado ainda.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
        ...
        <!-- Paginação -->
        <div class="pagination" id="guardians-paginacao">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-container">
                <form id="formGuardian" action="guardians.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-section">
                        <h3>Informações Profissionais e Parentesco</h3>
                        ...
                        <div class="file-input-container">
                            <label for="fotoperfil">Foto do Encarregado*</label>
...
<!-- Modal para visualizar encarregado -->
<div class="modal" id="view-guardian-modal">
    <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
            <div id="guardian-details">
                <!-- Detalhes do encarregado serão carregados via JavaScript -->
            </div>
            <div class="form-buttons">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (confirm("Tem certeza que deseja deletar este encarregado?")) {
                window.location.href = `delete_guardian.php?id=${id}`;
            }
        });
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Controle do modal de encarregado
    const guardianModal = document.getElementById('guardian-modal');
    const viewGuardianModal = document.getElementById('view-guardian-modal');
    ...
    // Abrir modal de adicionar encarregado
    if (addGuardianBtn) {
    ...
    closeModalBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            viewGuardianModal.style.display = 'none';
        });
    });
    
    // Fechar modal ao clicar fora
    window.addEventListener('click', function(e) {
        if (e.target === guardianModal) {
            guardianModal.style.display = 'none';
        }
        if (e.target === viewGuardianModal) {
            viewGuardianModal.style.display = 'none';
        }
    });
    ...
    const guardianForm = document.getElementById('formGuardian');
    ...
    // Função para visualizar encarregado
    function viewGuardian(id) {
        // Aqui você faria uma requisição AJAX para obter os detalhes do encarregado
            ...
            <div class="guardian-profile">
                <div class="guardian-header">
                    <div class="guardian-avatar">
                        <img src="uploads/guardians/default-avatar.jpg" alt="Foto do Encarregado">
                    </div>
                    <div class="guardian-basic-info">
                        <h2>Carlos Silva</h2>
                        <p><strong>ID:</strong> ${id}</p>
                        <p><strong>Parentesco:</strong> Pai</p>
                ...
                <div class="guardian-details-grid">
                    <div class="detail-section">
                        <h3>Informações Pessoais</h3>
                        <p><strong>Data de Nascimento:</strong> 15/05/1975</p>
                    ...
                    <div class="detail-section">
                        ...
                        <p><strong>Email:</strong> carlos.silva@email.com</p>
                    </div>
                    
                    <div class="detail-section">
                        <h3>Informações Profissionais</h3>
                        <p><strong>Profissão:</strong> Engenheiro</p>
                        <p><strong>Parentesco:</strong> Pai</p>
                        <p><strong>Aluno Vinculado:</strong> João Silva (Turma 8A)</p>
                    </div>
                    
                    <div class="detail-section">
                        <h3>Informações Adicionais</h3>
                        <p><strong>Data de Cadastro:</strong> 10/01/2020</p>
                        <p><strong>Observações:</strong> Responsável atento às atividades escolares.</p>
                    </div>
                </div>
            </div>
        `;
        
        document.getElementById('guardian-details').innerHTML = guardianDetails;
        viewGuardianModal.style.display = 'block';
    }
    
    // Função para editar encarregado
    function editGuardian(id) {
        // Aqui você faria uma requisição AJAX para obter os dados do encarregado
        ...
        document.getElementById('fname').value = 'Carlos';
        document.getElementById('lname').value = 'Silva';
        document.getElementById('dataa').value = '1975-05-15';
        ...
        document.getElementById('email').value = 'carlos.silva@email.com';
        document.getElementById('profissao').value = 'Engenheiro';
        document.getElementById('parentesco').value = 'Pai';
        document.getElementById('aluno_id').value = '1';
        
        // Exibir o modal
        guardianModal.style.display = 'block';
    }
    
    // Função para excluir encarregado
    function deleteGuardian(id) {
        if (confirm(`Tem certeza que deseja excluir o encarregado com ID ${id}?`)) {
            // Aqui você faria uma requisição AJAX para excluir o encarregado
            alert(`Encarregado com ID ${id} excluído com sucesso!`);
    ...
    const filterParentesco = document.getElementById('filter-parentesco');
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            filterGuardianTable(searchTerm, filterParentesco.value, filterStatus.value);
        });
    }
    ...
    if (resetFiltersBtn) {
        resetFiltersBtn.addEventListener('click', function() {
            searchInput.value = '';
            filterParentesco.value = '';
            filterStatus.value = '';
            filterGuardianTable('', '', '');
        });
    }
    
    function filterGuardianTable(searchTerm, parentesco, status) {
        const rows = document.querySelectorAll('#guardian-table-body tr');
        
        rows.forEach(row => {
            const nome = row.cells[1].textContent.toLowerCase();
            const parentescoCel = row.cells[2].textContent;
            const statusCel = row.cells[6].textContent.toLowerCase();
            
            const matchSearch = nome.includes(searchTerm);
            const matchParentesco = parentesco === '' || parentescoCel.includes(parentesco);
            const matchStatus = status === '' || statusCel.includes(status);
            
            if (matchSearch && matchParentesco && matchStatus) {
                row.style.display = '';
            } else {
    ...
    // Função para carregar coordenadores (simulação)
    function loadGuardians() {
        // Em um ambiente real, você faria uma requisição AJAX para obter os dados
        console.log("Carregando encarregados...");
          }
    
    // Carregar encarregados ao iniciar a página
    loadGuardians();
});
</script>
</body>
