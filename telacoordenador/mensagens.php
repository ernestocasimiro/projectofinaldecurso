<?php
    ...
    // Aqui pega o id do coordenador da sessão
...
<title>Mensagens - Dashboard de Coordenadores</title>
...
<style>
    ...
    .type-direcao { background-color: #795548; }
...
<script>
    // Dados simulados para mensagens de coordenadores
    const conversasSimuladas = [
        {
            ...
            ultimaMensagem: "Gostaria de marcar uma reunião sobre o comportamento do meu filho.",
            horario: "14:30",
            naoLida: true,
            prioridade: "media",
            mensagens: [
                {
                    id: 1,
                    conteudo: "Boa tarde, coordenador! Gostaria de marcar uma reunião sobre o comportamento do meu filho.",
                    enviada: false,
                    horario: "14:30",
                    data: "2025-04-15"
                }
        ...
        },
        {
            ...
            ultimaMensagem: "Preciso de orientação sobre um caso de indisciplina na turma 9A.",
            horario: "13:45",
            naoLida: false,
            prioridade: "alta",
            mensagens: [
                {
                    id: 1,
                    conteudo: "Coordenador, preciso de orientação sobre um caso de indisciplina na turma 9A.",
                    enviada: false,
                    horario: "13:45",
                    data: "2025-04-15"
                },
                {
                    id: 2,
                    conteudo: "Vamos agendar uma conversa com o aluno e os pais para resolvermos isso.",
                    enviada: true,
                    horario: "14:10",
                    data: "2025-04-15"
                }
        ...
        },
        {
            ...
            ultimaMensagem: "Relatório mensal precisa ser entregue até sexta-feira.",
            horario: "12:20",
            naoLida: true,
            prioridade: "alta",
            mensagens: [
                {
                    id: 1,
                    conteudo: "Relatório mensal precisa ser entregue até sexta-feira. Favor confirmar recebimento.",
                    enviada: false,
                    horario: "12:20",
                    data: "2025-04-15"
                }
        ...
        },
        {
            ...
            ultimaMensagem: "Lembrete: Reunião de pais e mestres na próxima quarta-feira.",
            horario: "11:15",
            naoLida: false,
            prioridade: "media",
            mensagens: [
                {
                    id: 1,
                    conteudo: "Lembrete: Reunião de pais e mestres na próxima quarta-feira às 18h.",
                    enviada: true,
                    horario: "11:15",
                    data: "2025-04-15"
                }
        ...
        },
        {
            ...
            ultimaMensagem: "Vamos alinhar o planejamento do próximo trimestre.",
            horario: "10:00",
            naoLida: false,
            prioridade: "media",
            mensagens: [
                {
                    id: 1,
                    conteudo: "Olá! Vamos alinhar o planejamento do próximo trimestre?",
                    enviada: false,
                    horario: "10:00",
                    data: "2025-04-15"
                },
                {
                    id: 2,
                    conteudo: "Claro! Podemos marcar para amanhã às 14h?",
                    enviada: true,
                    horario: "10:30",
                    data: "2025-04-15"
                }
    ...
    function openConversation(conversa) {
        ...
        document.getElementById('contactType').    // Renderizar mensagens
...
function renderMessages(mensagens) {
    ...
    mensagens.forEach(msg => {
        ...
        messageElement.className = `message-bubble ${msg.enviada ? 'sent' : 'received'}`;
        
        messageElement.innerHTML = `
            <div class="message-content">${msg.conteudo}</div>
            <div class="message-time">${msg.horario} - ${msg.data}</div>
            ${msg.enviada ? '<div class="message-status"><span class="material-symbols-outlined">done_all</span> Entregue</div>' : ''}
        `;
...
function filterMessages(filtro) {
    ...
    document.querySelectorAll('.messages-filters .filter-btn').forEach(btn => {
...
function sendMessage(event) {
    ...
    if (!messageText || !conversaAtiva) return;
    ...
    conversaAtiva.mensagens.push(newMessage);
    conversaAtiva.ultimaMensagem = messageText;
    conversaAtiva.horario = newMessage.horario;
    ...
    if (Math.random() > 0.3) { // 70% de chance de resposta
        setTimeout(() => {
            const replyMessages = [
                ...
                "Podemos marcar uma reunião para discutir isso?",
                "Recebido com sucesso."
            ];
            
            const reply = {
                id: conversaAtiva.mensagens.length + 1,
                conteudo: replyMessages[Math.floor(Math.random() * replyMessages.length)],
                enviada: false,
                horario: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                data: new Date().toLocaleDateString()
            };
            
            conversaAtiva.mensagens.push(reply);
            conversaAtiva.ultimaMensagem = reply.conteudo;
            conversaAtiva.horario = reply.horario;
            conversaAtiva.naoLida = true;
...
function toggleChatInfo() {
    alert("Informações detalhadas sobre a conversa serão exibidas aqui.");
}

function archiveConversation() {
    if (conversaAtiva) {
        if (confirm(`Deseja arquivar a conversa com ${conversaAtiva.nome}?`)) {
            alert(`Conversa com ${conversaAtiva.nome} arquivada com sucesso.`);
            // Em uma implementação real, você faria uma requisição para marcar a conversa como arquivada no banco de dados
        }
    } else {
...
function loadRecipients() {
    ...
    // Simular carregamento de destinatários do banco de dados
    const options = {
        'aluno': [
            { id: 1, name: 'João Pedro Silva (Turma 8A)' },
            { id: 2, name: 'Maria Eduarda Santos (Turma 9B)' },
            { id: 3, name: 'Carlos Alberto Oliveira (Turma 7C)' }
        ],
        'responsavel': [
            { id: 1, name: 'Ana Silva (Responsável por João Pedro)' },
            { id: 2, name: 'José Santos (Responsável por Maria Eduarda)' },
            { id: 3, name: 'Marta Oliveira (Responsável por Carlos Alberto)' }
        ],
        'professor': [
            { id: 1, name: 'Prof. Carlos Mendes (Matemática)' },
            { id: 2, name: 'Prof. Ana Paula (Português)' },
            { id: 3, name: 'Prof. Roberto (Ciências)' }
        ],
        'coordenacao': [
            { id: 1, name: 'Coord. Pedagógica' },
            { id: 2, name: 'Coord. Disciplinar' }
        ],
        'direcao': [
            { id: 1, name: 'Direção Escolar' }
        ],
        'turma': [
            { id: 1, name: 'Turma 7º Ano A' },
            { id: 2, name: 'Turma 8º Ano B' },
            { id: 3, name: 'Turma 9º Ano C' }
        ]
    };
...
function sendNewMessage(event) {
    ...
    alert(`Mensagem enviada com sucesso para ${recipients.length} destinatário(s):\n\nAssunto: ${subject}\nPrioridade: ${priority}\n\n${content}`);
...
function setupEventListeners() {
    // Habilitar/desabilitar botão de enviar conforme o conteúdo
    document.getElementById('messageInput').addEventListener('input', function() {
        document.getElementById('sendBtn').disabled = this.value.trim() === '';
    });
    
    // Auto-expandir textarea conforme o conteúdo
    document.getElementById('messageInput').addEventListener('input', function() {
        this.style.height = 'auto';
        this.style.height = (this.scrollHeight) + 'px';
    });
    
    // Adicionar tags para destinatários selecionados
    document.getElementById('recipients').addEventListener('change', function() {
        ...
        Array.from(this.selectedOptions).forEach(opt => {
            ...
            tag.innerHTML = `
                ${opt.textContent}
                <span class="remove-tag" onclick="removeRecipientTag(this, '${opt.value}')">×</span>
            `;
...
function removeRecipientTag(element, value) {
    const select = document.getElementById('recipients');
