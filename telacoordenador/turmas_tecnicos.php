...
// Buscar todos os cursos técnicos disponíveis
$stmtCursos = $conn->query("SELECT DISTINCT class_course FROM turma WHERE class_course LIKE 'Técnico%' ORDER BY class_course");
...
if ($cursoSelecionado) {
    ...
    // Buscar alunos disponíveis para cursos técnicos
    $stmtAlunos = $conn->prepare("
        SELECT e.id, CONCAT(e.fname, ' ', e.lname) as nome 
        FROM estudantes e
        WHERE e.area = 'Cursos Técnicos' AND NOT EXISTS (
...
<head>
    ...
    <title>Sistema Escolar - Cursos Técnicos</title>
...
<body>
    ...
    <main class="container">
        ...
        <h1 class="main-title"><?php echo $cursoSelecionado ? "Turmas do Curso de " . htmlspecialchars($cursoSelecionado) : "Cursos Técnicos"; ?></h1>
        
        <?php if (!$cursoSelecionado): ?>
            <div class="classes-container">
                <?php foreach ($cursos as $curso): ?>
                    <div class="class-card" onclick="window.location.href='turmas_tecnicos.php?curso=<?php echo urlencode($curso['class_course']); ?>'">
                        <h3>
                            <span class="material-symbols-outlined">
                                <?php 
                                    if (strpos($curso['class_course'], 'Informática') !== false) echo 'computer';
                                    elseif (strpos($curso['class_course'], 'Contabilidade') !== false) echo 'calculate';
                                    else echo 'medical_services';
                                ?>
                            </span>
                            <?php echo htmlspecialchars($curso['class_course']); ?>
                        </h3>
                        <p class="class-info">Clique para visualizar as turmas deste curso técnico</p>
        ...
        <?php else: ?>
            <div class="form-criar-turma">
                <h3>
                    <span class="material-symbols-outlined">
                        <?php 
                            if (strpos($cursoSelecionado, 'Informática') !== false) echo 'computer';
                            elseif (strpos($cursoSelecionado, 'Contabilidade') !== false) echo 'calculate';
                            else echo 'medical_services';
                        ?>
                    </span>
                    Criar Nova Turma
            ...
            <?php else: ?>
                <p class="no-turmas">Não há turmas cadastradas para este curso técnico.</p>
            <?php endif; ?>
